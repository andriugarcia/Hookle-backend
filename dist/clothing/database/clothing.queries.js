...
var random = function (filter) { return "MATCH ()-[r:LIKES]->(c:Clothing)\n                            WITH c, count(r) as rel_cnt\n                            WHERE rel_cnt <= 5 " + filter + "\n                            WITH c, rand() AS number\n                            RETURN c\n                            ORDER BY number\n                            LIMIT 5"; };
var getFavProduct = "MATCH (u:User {email: $emailParam})-[:FAVORITE]->(c:Clothing {code: $clothingParam}) RETURN c";
